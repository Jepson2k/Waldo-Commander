...
async def _run_cycle_start(page_client: Client) -> None:
    ...
    editor widgets and notifies on that page.
    """
    if page_client._deleted or page_client.id not in Client.instances:
        return
    try:
        with page_client:
...
def _automation_tick(page_client: Client | None) -> None:
    ...
    Reads the published ``commander.status`` surface (one tick behind the
    wire — irrelevant at status rate) so the watchers see exactly what every
    other public consumer sees. Both watchers are edge-detecting: they act
    only on transitions, never per tick.
    """
    a = automation_state
    st = waldoctl.commander.status
    io = st.io

    # Cycle start: rising edge on Input 1 runs the active program. The edge
    # tracker runs even while disabled so enabling with the input held high
    # never fires until a fresh low→high transition. ``io`` is only published
    # while a page is connected and this watcher reads one tick behind the
    # publish, so the tracker only trusts values seen after a full live tick —
    # a press latched while headless (or held high at launch) never fires.
    ...
    if page_client is None:
        ...
        if not fresh or a._cycle_prev_input < 0:
            ...
            if (
                ...
                asyncio.create_task(_run_cycle_start(page_client))

    # Home output: Output 2 mirrors "all joints within tolerance of the home
    # pose". ``status.homed`` is only the referencing flag, so the distance is
    ...
    if a.home_output_enabled or a._home_out_on:
        desired = False
        if a.home_output_enabled:
            ...
            desired = dist <= band
        if desired != a._home_out_on:
            a._home_out_on = desired
            asyncio.create_task(_write_home_output(1 if desired else 0))
            ...
            try:
                ...
                with global_phase_timer.phase("status"):
                    ...
                    if pc is not None and (
                        pc._deleted or pc.id not in Client.instances
                    ):
                        pc = None
